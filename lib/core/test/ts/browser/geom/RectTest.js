...
UnitTest.asynctest('browser.tinymce.core.geom.RectTest', function () {
    ...
    suite.test('relativePosition', function () {
        var sourceRect = Rect.create(0, 0, 20, 30), targetRect = Rect.create(10, 20, 40, 50), tests = [
        ...
        Tools.each(tests, function (item) {
            LegacyUnit.deepEqual(Rect.relativePosition(sourceRect, targetRect, item[0]), Rect.create(item[1], item[2], item[3], item[4]), item[0]);
        });
    });
    suite.test('findBestRelativePosition', function () {
        ...
        Tools.each(tests, function (item) {
            LegacyUnit.equal(Rect.findBestRelativePosition(sourceRect, targetRect, Rect.create(item[1], item[2], item[3], item[4]), item[0]), item[5], item[5]);
        });
    });
