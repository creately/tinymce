...
UnitTest.asynctest('browser.tinymce.core.CaretContainerRemoveTest', function () {
    ...
    var sTestRemove = Logger.t('Remove', Step.sync(function () {
    ...
    var sTestRemoveAndRepositionBlockAtOffset = Logger.t('removeAndReposition block in same parent at offset', Step.sync(function () {
        setupHtml('<span contentEditable="false">1</span>');
        ...
        var pos = CaretContainerRemove.removeAndReposition(getRoot().firstChild, CaretPosition(getRoot(), 0));
    ...
    var sTestRemoveAndRepositionBeforeOffset = Logger.t('removeAndReposition block in same parent before offset', Step.sync(function () {
        ...
        var pos = CaretContainerRemove.removeAndReposition(getRoot().childNodes[1], CaretPosition(getRoot(), 0));
        Assertions.assertEq('Should be unchanged offset', 0, pos.offset());
        Assertions.assertDomEq('Should be unchanged container', Element.fromDom(getRoot()), Element.fromDom(pos.container()));
        Assertions.assertEq('Should not be block container', false, CaretContainer.isCaretContainerBlock(getRoot().childNodes[1]));
    }));
    var sTestRemoveAndRepositionAfterOffset = Logger.t('removeAndReposition block in same parent after offset', Step.sync(function () {
        setupHtml('<span contentEditable="false">1</span><span contentEditable="false">2</span>');
        CaretContainer.insertBlock('p', getRoot().childNodes[1], true);
        Assertions.assertEq('Should be block container', true, CaretContainer.isCaretContainerBlock(getRoot().childNodes[1]));
        var pos = CaretContainerRemove.removeAndReposition(getRoot().childNodes[1], CaretPosition(getRoot(), 3));
...
//# sourceMappingURL=CaretContainerRemoveTest.js.map