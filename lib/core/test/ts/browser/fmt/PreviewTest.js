...
UnitTest.asynctest('browser.tinymce.core.fmt.PreviewTest', function () {
    ...
    suite.test('Get preview css text for formats', function (editor) {
        ...
        ok(!/font-weight\:(bold|700)/.test(getCssText({ inline: 'b', preview: 'font-size' })), 'Bold should not be when we only preview font-size');
        ok(/color\:rgb\(255, 0, 0\)/.test(getCssText({ inline: 'custom', styles: { color: '#ff0000' } })), 'Test preview of a custom element.');
        ok(/color\:rgb\(255, 0, 0\)/.test(getCssText({ inline: 'invalid', styles: { color: '#ff0000' } })), 'Test preview of an invalid element shouldn\'t crash the editor .');
        ok(/color\:rgb\(0, 255, 0\)/.test(getCssText({ selector: 'tr', classes: ['preview'] })), 'Style is properly inherited in preview for partial element (like TR).');
        ok(/color\:rgb\(255, 0, 0\)/.test(getCssText({ selector: 'li', classes: ['preview'] })), 'For LI element default required parent is UL.');
        ok(/color\:rgb\(0, 0, 255\)/.test(getCssText({ selector: 'ol li', classes: ['preview'] })), 'Parent explicitly present in the selector will have preference.');
    ...
    TinyLoader.setup(function (editor, onSuccess, onFailure) {
        Pipeline.async({}, Arr.flatten([
            [
                ...
                Waiter.sTryUntil('Expected styles where not loaded', Step.sync(function () {
...
//# sourceMappingURL=PreviewTest.js.map