...
UnitTest.asynctest('browser.tinymce.plugins.toc.TocPluginTest', function () {
    ...
    suite.test('mceInsertToc', function (editor) {
        ...
        LegacyUnit.equal(trimBr(HtmlUtils.normalizeHtml($toc[0].outerHTML)), '<div class="tst-toc" contenteditable="false">' +
            ...
            '</li>' +
            '</ul>' +
            '</div>', 'no surprises in ToC structure');
    });
    suite.test('mceInsertToc - flat structure', function (editor) {
        ...
        var $toc = editor.$('.tst-toc');
        stripAttribs($toc, ['data-mce-href', 'data-mce-selected']);
        LegacyUnit.equal(trimBr(HtmlUtils.normalizeHtml($toc[0].innerHTML)), '<h3 contenteditable="true">Table of Contents</h3>' +
            '<ul>' +
            '<li>' +
            '<a href="#h1">H1</a>' +
            '</li>' +
            ...
            '</ul>' +
            '</li>' +
            '</ul>', 'no surprises in ToC structure');
    });
    ...
    suite.test('Misc.', function (editor) {
        ...
        LegacyUnit.equal(trimBr(HtmlUtils.normalizeHtml($toc[0].innerHTML)), '<h3 contenteditable="true">Table of Contents</h3>' +
            ...
            '<a href="#h2">H2</a>' +
            '</li>' +
            '</ul>', 'the largest available header becomes first ToC level');
    });
...
//# sourceMappingURL=TocPluginTest.js.map