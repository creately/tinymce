...
UnitTest.asynctest('tinymce.lists.browser.ApplyTest', function () {
    ...
    suite.test('Apply OL to UL and DO not merge with adjacent lists because styles are different (original has style)', function (editor) {
        editor.getBody().innerHTML = LegacyUnit.trimBrs('<ol style="list-style-type: upper-roman;">' +
            '<li>a</li>' +
            '</ol>' +
            '<ul><li>b</li></ul>' +
        ...
        LegacyUnit.setSelection(editor, 'ul li', 1);
        LegacyUnit.execCommand(editor, 'InsertOrderedList');
        LegacyUnit.equal(editor.getContent(), '<ol style="list-style-type: upper-roman;">' +
            '<li>a</li>' +
            '</ol>' +
            '<ol><li>b</li></ol>' +
            '<ol style="list-style-type: upper-roman;">' +
    ...
    suite.test('Apply OL to UL should merge with above list because styles are the same (both have lower-roman), but not below list', function (editor) {
        editor.getBody().innerHTML = LegacyUnit.trimBrs('<ol style="list-style-type: lower-roman;">' +
            '<li>a</li>' +
            '</ol>' +
            '<ul><li>b</li></ul>' +
            '<ol style="list-style-type: upper-roman;">' +
        ...
        LegacyUnit.equal(editor.getContent(), '<ol style="list-style-type: lower-roman;">' +
            '<li>a</li>' +
            '<li>b</li>' +
            '</ol>' +
            '<ol style="list-style-type: upper-roman;">' +
    ...
    suite.test('Apply OL to UL should merge with below lists because styles are the same (both have roman), but not above list', function (editor) {
        editor.getBody().innerHTML = LegacyUnit.trimBrs('<ol style="list-style-type: upper-roman;">' +
            '<li>a</li>' +
            '</ol>' +
            '<ul><li>b</li></ul>' +
    ...
    suite.test('Apply OL to UL and DO not merge with adjacent lists because classes are different', function (editor) {
        editor.getBody().innerHTML = LegacyUnit.trimBrs('<ol class="a">' +
            '<li>a</li>' +
            '</ol>' +
            '<ul><li>b</li></ul>' +
            '<ol class="b">' +
        ...
        LegacyUnit.setSelection(editor, 'ul li', 1);
        LegacyUnit.execCommand(editor, 'InsertOrderedList');
        LegacyUnit.equal(editor.getContent(), '<ol class="a">' +
            '<li>a</li>' +
            '</ol>' +
            '<ol><li>b</li></ol>' +
            '<ol class="b">' +
    ...
    suite.test('Apply UL list to multiple formatted lines separated by BR', function (editor) {
        editor.settings.forced_root_block = false;
        editor.getBody().innerHTML = ('<strong>a</strong><br>' +
            '<span>b</span><br>' +
            '<em>c</em>');
        editor.focus();
    ...
    suite.test('Handle one empty unordered list items without error', function (editor) {
        ...
        LegacyUnit.execCommand(editor, 'InsertUnorderedList');
        LegacyUnit.equal(editor.getBody().innerHTML, '<p>a</p>' +
            '<p>b</p>' +
    ...
    TinyLoader.setup(function (editor, onSuccess, onFailure) {
        ...
        indent: false,
        entities: 'raw',
        valid_elements: 'li[style|class|data-custom|data-custom1|data-custom2],ol[style|class|data-custom|data-custom1|data-custom2],' +
...
//# sourceMappingURL=ApplyTest.js.map